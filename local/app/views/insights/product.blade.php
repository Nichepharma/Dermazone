...
@section('content')
    <div class="page-content1">
        ...
        <div class='col-md-10 col-xs-12'>
            ...
            <div class="container">
                @if($data['types'])
                    @foreach($data['types'] as $typeNum=>$type)
                        <div class="col-md-2 col-xs-6">
                            <a class="buttonallsite3" @if($typeNum==1) href="{{url('insights/product-doctors/'.$data['productId'])}}" @endif>{{$type}}</a>
                        </div>
                    @endforeach
                @endif
            </div>
            ...
            <div class="graphContainer">
                <h2 class="graphTitle"> Calls per segment for - {{$data['products'][$data['productId']]}} </h2>
